...
if __name__ == '__main__':
    ...
    for i in range(len(xfast)):
        ...
        graf_x_t = plt.figure('x(t) ' +str(i+1),figsize=(12,3))
        plt.grid()
        plt.title('X-posisjon over tid for måling ' + str(i+1))
        plt.xlabel('Tid ($s$)')
        plt.ylabel('X posisjon ($m$)')
        plt.plot(time[i], xfast[i], label='Eksperimentell x(t)')
        plt.plot(time[i], xfast[i], '*', label='Målepunkter')
        plt.plot(tNumeric,xNumeric, 'r', linestyle='dashed', label='Numerisk x(t)')
        plt.legend(loc="lower center")
        graf_x_t.savefig("img/x_av_t_maaling_"+str(i+1)+".svg", bbox_inches='tight')
